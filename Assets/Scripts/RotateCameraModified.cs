...
public class RotateCameraModified : MonoBehaviour
{
    public float lookLeftMax = -8.651f;
    public float lookRightMax = 7.651f;
    Quaternion resetCamera;
    ...
    void moveCamera(float horizontal, float verticle, float moveSpeed)
    {
        ...
        rotY += mouseX * moveSpeed/10;
        rotX += mouseY * moveSpeed/10;

        rotY = Mathf.Clamp(rotY, lookLeftMax, lookRightMax);
        localRotation = Quaternion.Euler(0.0f, rotY, -rotY);
